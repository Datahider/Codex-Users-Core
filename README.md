...
- каждое окно отображается полосой из 10 символов: `█` показывает оставшуюся
  долю, `░` — использованную; количество заполненных сегментов округляется до
  ближайших 10 процентов;
- строка окна имеет вид `<окно>  <полоса>  <остаток>% · сброс <дата и время>`;
