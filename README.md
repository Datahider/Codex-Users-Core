...
с номером слота, PID и настроенным пределом. Записи `Manager worker slot state`
перед началом и после завершения каждого события дополнительно содержат точный
путь, inode, device и результат независимой попытки захвата через второй
дескриптор. При `max_workers = 1` сохраняется
