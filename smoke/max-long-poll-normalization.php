...
if (!is_array($fixture)) {
    fwrite(STDERR, "Invalid JSON fixture: {$fixturePath}\n");
    exit(1);
}

$tmpRoot = sys_get_temp_dir() . '/codex-runtime-max-poll-smoke-' . bin2hex(random_bytes(4));
...
try {
    ...
    assertSame(true, $result['accepted'] ?? null, 'update should be accepted');
    assertNotEmpty($result['event_id'] ?? null, 'event id should be returned');
    assertSame(null, $result['reason'] ?? null, 'accepted update should not include a reason');
    ...
    fwrite(STDOUT, "MAX long-poll normalization smoke: OK\n");
    fwrite(STDOUT, "Fixture: {$fixturePath}\n");
    fwrite(STDOUT, "Queued event: {$eventPath}\n");
    exit(0);
} catch (Throwable $e) {
    fwrite(STDERR, "MAX long-poll normalization smoke failed: {$e->getMessage()}\n");
    exit(1);
} finally {
    deleteTree($tmpRoot);
}
